...
public class ImageSimilarTest {
	 ...
	 */
	public static void searchImage(int n, String str1, String str2,
		...
		for (int j = 0; j < n; j++) {
			float f = 0f;
			for (int i = 0; i < characNum; i++) {
				charact2[i] = idents[i].getCharacteristic(str2 + (j + 1)
						+ ".jpg");
		...
		for (int i = 0; i < n; i++) {
			...
			ImageDigital.writeImg(img, "jpg", str2 + "T" + element[i].similar
					+ "_" + element[i].index + ".jpg");
		}
	}
	...
	public static void processingTest(String str1, String str2) {
		...
		/*
		 * int sum = 0; for(int i=0; i<w*h; i++) { sum += pix[i]; } int avgPix =
		 * (int)(sum/(w*h)); System.out.println("sum:" + sum + "  avgPix:" +
		 * avgPix); StringBuilder sb = new StringBuilder(); for(int i=0; i<w*h;
		 * i++) { if(pix[i] >= avgPix) { pix[i] = 1; } else { pix[i] = 0; } }
		 * long result = 0; if(sb.charAt(0) == '0') { result =
		 * Long.parseLong(sb.toString(), 2); } else {
		 * //如果第一个字符是1，则表示负数，不能直接转换成long， result = 0x8000000000000000l ^
		 * Long.parseLong(sb.substring(1), 2); }
		 */
		// BufferedImage img = new BufferedImage(DCTW, DCTH,
	...
	public static void main(String[] args) {
		...
		String str1 = "F:\\image processing\\测试图片素材\\测试图片3.2\\Baboo.bmp";
