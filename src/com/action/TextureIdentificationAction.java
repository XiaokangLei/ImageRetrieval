...
/**
 * @author 小康
 * @date： 日期：Apr 17, 2017 时间：9:35:12 AM
 */
public class TextureIdentificationAction extends ActionSupport {
	...
	public String TextureIdentification() throws Exception {
		...
		for (i = 0; i < tempList.length; i++) {
			if (tempList[i].isFile()) {
				...
				// + strArray.get(i));
				int[] vector1 = image.transformation.UniformLBP
						.getFeatureVector(tempList[i].toString());
				samelist.add(image.transformation.UniformLBP
		...
		for (Double key : map.keySet()) {
			...
			System.out.println(samerateStrings.get(k) + " 均值纹理  " + value);
			k++;
		}
